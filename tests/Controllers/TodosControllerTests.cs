...
namespace todo_api.Tests.Controllers;
...
public class TodosControllerTests : IClassFixture<TodoApiFactory>
{
    ...
    {
        await SeedAsync(new TodoItem { Title = "Seeded", CreatedAt = DateTimeOffset.UtcNow, UpdatedAt = DateTimeOffset.UtcNow });
    ...
    public async Task Create_Returns201_WithLocation()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/todos", new CreateTodoDto { Title = "Integration test todo" });
    ...
    {
        await SeedAsync(new TodoItem { Title = "To delete", CreatedAt = DateTimeOffset.UtcNow, UpdatedAt = DateTimeOffset.UtcNow });
        var client = CreateClient();
